...
**检查时机是 flush，不在写路径上。** 整理要把整个文件重抄一遍，若跟在某个 bucket 落盘之后触发，
就等于「保存一个区块」突然变成「重写几 MB」，正是玩家最敏感的时刻。`CsoStorage.flush()` 每次最多
整理一个文件，并按 LRU 从最冷的开始——一个碎片化的存档会在几次自动保存内陆续恢复，而不是把一次
自动保存卡成长久停顿。
